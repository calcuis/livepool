...
from database import Poll, Option, create_tables, get_db
...
def vote(poll_id: int, body: VoteRequest, db: Session = Depends(get_db)):
    poll = db.query(Poll).filter(Poll.id == poll_id).first()
    if not poll:
        raise HTTPException(status_code=404, detail="Poll not found.")
...
def end_poll(poll_id: int, db: Session = Depends(get_db)):
    poll = db.query(Poll).filter(Poll.id == poll_id).first()
    if not poll:
        raise HTTPException(status_code=404, detail="Poll not found.")
...
# ── Frontend Integration (For Tunneling / Production) ─────────────────────────
...
if os.path.isdir(DIST_DIR):
    # Mount the /assets static folder which Vite creates
    app.mount("/assets", StaticFiles(directory=os.path.join(DIST_DIR, "assets")), name="assets")

    # Add a catch-all route for SPA routing so Vite handles the routing
    @app.get("/{full_path:path}")
    async def serve_frontend(full_path: str):
        # Prevent any static file serving from overlapping with /polls API calls
        if full_path.startswith("polls/"):
            raise HTTPException(status_code=404, detail="API route not found")
        
        # Try to serve a specific file if it exists (e.g., vite.svg)
